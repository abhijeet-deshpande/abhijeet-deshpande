# Hi, I'm Abhijeet Deshpande 👋

### Platform & DevOps Engineer | 10+ Years of Enterprise Experience

**AWS · Kubernetes · Terraform · Ansible · CI/CD · Grafana · Docker · Python**

🌍 Based in India · Open to DevOps / Platform Engineering roles in **Germany, Netherlands, Austria, UK & EU**

🌐 Web: [Click Me](https://abhijeet-deshpande.github.io/abhijeet-deshpande/) 🚀 | [![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/abhijeetd04) | [![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:connect.abhijeet01@gmail.com) | [![GitHub](https://img.shields.io/badge/GitHub-000000?style=flat&logo=github&logoColor=white)](https://abhijeet-deshpande.github.io/abhijeet-deshpande) | [![Profile Views](https://komarev.com/ghpvc/?username=abhijeet-deshpande&label=Profile+Views&color=0077B5&style=flat)](https://github.com/abhijeet-deshpande)


---

## 👨‍💻 About Me

Platform & DevOps Engineer with **10+ years of enterprise experience** in infrastructure automation, compliance, observability, and cloud operations. I have worked extensively on large-scale environments involving BMC TSSA, BMC TSO, automated patching, CI/CD pipelines, and Grafana-based monitoring across hybrid infrastructure.

Currently upskilling in **AWS, Kubernetes, Terraform, and Python** to move into cloud-native platform engineering roles in the **EU / Germany** market.

- 🔭 Currently building: Cloud & Kubernetes portfolio projects
- 🌱 Currently learning: AWS (SAA-C03), CKA, Terraform advanced patterns
- 🎯 Target roles: DevOps Engineer · Platform Engineer · SRE · Cloud Engineer
- 📍 Location: India → Targeting Germany / EU relocation
- 💬 Ask me about: Enterprise automation, patching pipelines, Grafana dashboards, BMC tooling, CI/CD, Kubernetes, Docker, Terraform

---

## 🛠️ Tech Stack
<p align="left">
<img src="https://skillicons.dev/icons?i=aws,azure,gcp,docker,kubernetes,terraform,ansible,grafana,prometheus,python,flask,bash,powershell,gitlab,github" />
</p>

| Category | Tools & Technologies |
|---|---|
| ☁️ Cloud | AWS (EC2, S3, VPC, IAM, EKS, Lambda, CloudWatch,Route53, ALB) |
| 📦 Containers & Orchestration | Docker, Kubernetes, Helm, EKS |
| 🏗️ Infrastructure as Code | Terraform, Ansible |
| 🔁 CI/CD | GitHub Actions, GitLab CI, YAML Pipelines |
| 📊 Observability | Grafana, Prometheus |
| 🔐 Compliance & Patching | BMC TSSA, BMC TSO, Automated Patching |
| 🐧 OS & Scripting | Linux (RHEL/CentOS/Ubuntu), Bash, Python, Shell |
| 🗃️ Version Control | Git, GitHub, GitLab |
| 📋 ITSM | Incident & Change Management |

---

## 🎓 Certifications

| Certification | Issuer | Status | Credential |
|---|---|---|---|
| AWS Solutions Architect Associate (SAA-C03) | AWS | 🔄 In Progress | — |
| AWS Developer Associate | AWS | 🔄 Planned | — |
| Certified Kubernetes Administrator (CKA) | CNCF | 🔄 Planned | — |
| HashiCorp Certified: Terraform Associate (004) | Hashicorp | 🔄 Planned | — |
| KCNA | CNCF | 🔄 Planned | — |
| AWS Certified Cloud Practitioner | ISSUER | ✅ Certified |  [View Certifictaion](https://www.credly.com/badges/8564b8be-3b96-4e3e-b60c-c9ad688558ee/linked_in_profile)|

---

## 🚀 Featured Projects

### 🔧 [AWS Infrastructure Automation](https://github.com/YOUR_GITHUB_USERNAME/aws-infra-automation)
> Terraform + Ansible multi-environment AWS setup (dev/staging/prod) with EKS cluster, GitHub Actions CI/CD pipeline, and Grafana + Prometheus monitoring stack.

`Terraform` `Ansible` `AWS` `EKS` `GitHub Actions` `Grafana` `Prometheus`

---

### 🛡️ [Compliance & Patching Automation](https://github.com/YOUR_GITHUB_USERNAME/compliance-patching-automation)
> Automated Linux server patching and compliance drift detection using Ansible and Python. Includes a Grafana dashboard for real-time patch status visibility across multiple servers.

`Ansible` `Python` `Bash` `Grafana` `Linux` `Compliance`

---

### 📚 [DevOps Labs](https://github.com/YOUR_GITHUB_USERNAME/devops-labs)
> Hands-on DevOps lab tasks completed on the KodeKloud Engineer platform — Linux, Docker, Kubernetes, Terraform, and AWS. Each task documented with problem statement, commands, and proof of completion.

`Linux` `Docker` `Kubernetes` `Terraform` `AWS`

---

## 📈 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=abhijeet-deshpande&show_icons=true&theme=dark&hide_border=true&count_private=true" height="165"/>
<img src="https://github-readme-streak-stats.herokuapp.com?user=abhijeet-deshpande&theme=dark&hide_border=true" height="165"/>

</div>

<div align="center">

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=abhijeet-deshpande&layout=compact&theme=dark&hide_border=true&langs_count=8"/>

</div>

---

## 🌍 Open to Opportunities

I am actively seeking roles in:

| | |
|---|---|
| 🎯 **Roles** | DevOps Engineer, Platform Engineer, SRE, Cloud Engineer, Infrastructure Engineer, Lead DevOps Engineer, Senior Site Reliability Engineer|
| 🌍 **Locations** | Germany 🇩🇪 · Netherlands 🇳🇱 · Austria 🇦🇹 · Switzerland 🇨🇭 · UK 🇬🇧 · EU Remote 🌐 |
| ⏱️ **Notice Period** | 90 days |
| 🛂 **Visa** | Require sponsorship · Eligible for EU Blue Card |
| 📬 **Contact** | connect.abhijeet01@gmail.com |

---

## 📬 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/abhijeetd04)
[![Email](https://img.shields.io/badge/Email-Reach%20Out-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:connect.abhijeet01@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/abhijeet-deshpande)

</div>

---

<div align="center">
  <i>"Automate everything. Document everything. Ship consistently."</i>
</div>
